...
def carnumber_feature(img_ori):
    ...
    for i, plate_img in enumerate(plate_imgs):
        ...
        chars = receiveImg(img_result, '88ba82fedc172fa6718efbf18cb0eac2')  # words 는 번호판인식 함수
        result_chars = ''
        has_digit = False
        for c in chars:
    ...
    crop_img = img_ori[info['y']:info['y']+info['h'],info['x']:info['x']+info['w']]
    word = receiveImg(crop_img, '88ba82fedc172fa6718efbf18cb0eac2')  # words 는 번호판인식 함수
    word = word.replace(" ","")
    print("읽어온 번호 : ",word)
    if (len(word) == 7 or len(word) == 8 ):
        return crop_img, word
    else:
        return 0,0